		...
		if( get_field( 'security_deposit', $lead->ID ) ){ $security = get_field( 'security_deposit', $lead->ID ); }
			...
			echo "<br>Deposit--->$" . $deposit; //first + last + security
					...
					<?php 
						...
						/*if( $due < $deposit ){ $due = $deposit; }
						else{ $due = ((($weeks-1) * $rate)+($security + $rate + $app_fee)); }
						echo "<br>DUE--->$" . $due;
						*/
					...
					<div class='col-md-2'>
						<button id='moveout<?php echo $lead->ID; ?>' class='btn btn-default btn-block'>Move Out</button>
				...
				<?php
					...
					echo 'Phone#: ' . get_field( 'lead_phone', $lead->ID ); 
